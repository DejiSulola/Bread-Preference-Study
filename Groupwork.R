...
print("Here we run our conditional logit model for all the database")
...
print("Then we estimate the WTP for everyone")
wtp.gmnl(Models_Non_Linear.cl, wrt = "pr")

### Run a Conditional Logit Model with non-linear effects for all variables in Treated group
print("Here we run our conditional logit model for only treated group")
Models_Non_Linear_treated.cl <-
  gmnl(
    formula = Choice ~ wr + ww + rr + nf + ns + nsf + pr + asc_a + asc_b|0,
    data = Treated_choice_data_mlogit,
    reflevel = "3"
  )
summary(Models_Non_Linear_treated.cl)

print("Here we run our WTP estimation for only treated group")
wtp.gmnl(Models_Non_Linear_treated.cl, wrt = "pr")

### Run a Conditional Logit Model with non-linear effects for all variables in Treated group
print("Here we run our conditional logit model for only non-treated group")
Models_Non_Linear_nontreated.cl <-
  gmnl(
    formula = Choice ~ wr + ww + rr + nf + ns + nsf + pr + asc_a + asc_b|0,
    data = Non_Treated_choice_data_mlogit,
    reflevel = "3"
  )
summary(Models_Non_Linear_nontreated.cl)

print("Here we run our WTP estimate for only non-treated group")
wtp.gmnl(Models_Non_Linear_nontreated.cl, wrt = "pr")



### Run the Random Parameter Model with Unobserved Heterogeneity for treated
print("Here we run our Mixed conditional logit model for only Treated group")
Models_Non_Linear_treatment.MIXL <-
  gmnl(
    formula = Choice ~ wr + ww + rr + nf + ns + nsf + pr + asc_a + asc_b | 0 ,
    data = Treated_choice_data_mlogit,
    model = "mixl",
...
summary(Models_Non_Linear_treatment.MIXL)

#compute willingness to pay estimates based on the Mixed logit Model
print("Here we run our WTP estimation for only Treated group in the Mixed model")
wtp.gmnl(Models_Non_Linear_treatment.MIXL, wrt = "pr")

### Run the Random Parameter Model with Unobserved Heterogeneity for Non-treated
print("Here we run our Mixed conditional logit model for only Non-treated group")
Models_Non_Linear_nontreatment.MIXL <-
  gmnl(
    formula = Choice ~ wr + ww + rr + nf + ns + nsf + pr + asc_a + asc_b | 0 ,
    data = Non_Treated_choice_data_mlogit,
    model = "mixl",
...
summary(Models_Non_Linear_nontreatment.MIXL)

#compute willingness to pay estimates based on the Mixed logit Model

print("Here we run our WTP estimation for only Non-treated group in the Mixed model")
wtp.gmnl(Models_Non_Linear_nontreatment.MIXL, wrt = "pr")
################
Models_Non_Linear_treatedhetero.MIXL <-
  gmnl(
    formula = Choice ~ wr + ww + rr + nf + ns + nsf + pr + asc_a + asc_b | 0 | 0 | int_student  + Gender + income,
    data = Treated_choice_data_mlogit,
    model = "mixl",
    ranp = c(
      wr = "n",
    ...
    mvar = list(
      wr = c("int_student", "Gender", "income"), 
      ww = c("int_student", "Gender", "income"),
      rr = c("int_student", "Gender", "income"),
      nf = c("int_student", "Gender", "income"),
      ns = c("int_student", "Gender", "income"),
      nsf = c("int_student", "Gender", "income"),
      pr = c("int_student", "Gender", "income")
    ),
    reflevel = "3",
    R = 1000,
    panel = TRUE,
...
summary(Models_Non_Linear_treatedhetero.MIXL)

print("WTP")
wtp.gmnl(Models_Non_Linear_treatedhetero.MIXL, wrt = "pr") 
  ############################################################
print("Then we look at preference and WTP for non-treated")
Models_Non_Linear_nontreatedhetero.MIXL <-
  gmnl(
    formula = Choice ~ wr + ww + rr + nf + ns + nsf + pr + asc_a + asc_b | 0 | 0 | int_student  + Gender + income,
    data = Non_Treated_choice_data_mlogit,
    model = "mixl",
...
summary(Models_Non_Linear_nontreatedhetero.MIXL)

print("WTP")
wtp.gmnl(Models_Non_Linear_nontreatedhetero.MIXL, wrt = "pr") 
###############

AIC(Models_Non_Linear_treatment.MIXL)
AIC(Models_Non_Linear_nontreatment.MIXL)
AIC(Models_Non_Linear_treatedhetero.MIXL)
AIC(Models_Non_Linear_nontreatedhetero.MIXL)


library(skimr)
skim(Treated_Database$income)
skim(Non_Treated_Database$income)
skim(Treated_Database$Gender)
skim(Non_Treated_Database$Gender)
skim(Treated_Database$int_student)
skim(Non_Treated_Database$int_student)
skim(Treated_Database$income)
skim(Non_Treated_Database$income)
...
winners <- read.csv("survey_lottery_emails (1).csv", header = TRUE)
winners <- bread_data %>%
  add_column(id = 1:188,
             .before = 1)
